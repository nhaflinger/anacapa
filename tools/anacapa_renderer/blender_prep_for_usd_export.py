...
class _MtlxBuilder:
    ...
    def _translate_node(self, bl_node):
        ...
        if t == 'BSDF_PRINCIPLED':
            ...
            out = self._tx_bsdf_hair_principled(bl_node)
        elif t == 'BSDF_HAIR':
    ...
    def _tx_bsdf_hair_principled(self, n):
        """Principled Hair BSDF → open_pbr_surface approximation.

        Maps Blender's Principled Hair BSDF to an open_pbr_surface node so that
        the anacapa renderer can extract the hair color and roughness parameters.
        The renderer's OslMaterial class detects ctx.isCurve at shading time and
        delegates to its built-in MarschnerHairMaterial using:
          - base_color          → sigma_a (Beer-Lambert inversion)
          - specular_roughness  → beta_m (longitudinal roughness)
          - specular_anisotropy → beta_n_ratio (azimuthal roughness factor)
          - specular_ior        → IOR (cortex refractive index)

        For Melanin Concentration mode the Color input is None; we fall back to
        a medium-brown default (0.40, 0.25, 0.10) which gives visually reasonable
        hair when no per-strand color override is present.  Per-strand colors from
        the AHAIR002 binary format take priority at shading time.
        """
        node = self._ng.addNode('open_pbr_surface', self._uid('openpbr'), 'surfaceshader')
        node.addInput('base_weight', 'float').setValue(1.0)

        # Color — Direct Coloring mode uses 'Color'; Melanin mode uses 'Melanin' (no color3 socket).
        # Fall back to medium brown when the Color socket is absent or disconnected.
        col_out, col_c = self._socket_color3(n.inputs.get('Color'), (0.40, 0.25, 0.10))
        c = node.addInput('base_color', 'color3')
        if col_out:
            self._connect(c, col_out)
        else:
            c.setValueString(f'{col_c[0]}, {col_c[1]}, {col_c[2]}')

        # Longitudinal roughness → specular_roughness
        rough_out, rough_c = self._socket_float(n.inputs.get('Roughness'), 0.40)
        r = node.addInput('specular_roughness', 'float')
        if rough_out:
            self._connect(r, rough_out)
        else:
            r.setValue(float(rough_c))

        # Radial (azimuthal) roughness → specular_anisotropy (abused as a passthrough).
        # The OslMaterial hair path reads xalpha vs yalpha from the GGX lobe to
        # recover beta_n.  If not connected, default to slightly wider than beta_m.
        rad_out, rad_c = self._socket_float(n.inputs.get('Radial Roughness'), 0.60)
        a = node.addInput('specular_anisotropy', 'float')
        if rad_out:
            self._connect(a, rad_out)
        else:
            a.setValue(float(rad_c))

        # IOR (cortex refractive index, ~1.55 for human hair)
        ior_out, ior_c = self._socket_float(n.inputs.get('IOR'), 1.55)
        i = node.addInput('specular_ior', 'float')
        if ior_out:
            self._connect(i, ior_out)
        else:
            i.setValue(float(ior_c))
...
def _inject_sss_params(usd_path):
    ...
    for prim in stage.Traverse():
        ...
        strength = getattr(mat, "anacapa_subsurface_strength", 1.0)
        ...
        for desc in Usd.PrimRange(prim):
            ...
            shader.CreateInput("anacapa_subsurface_strength", Sdf.ValueTypeNames.Float).Set(strength)
            touched += 1
